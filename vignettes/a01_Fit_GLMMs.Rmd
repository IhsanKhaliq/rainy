...
summary(mod1)
```
The model summary suggest that there are not enough degrees of freedom to fit a complicated model. We can drop relative humidity and temperature because we directly transferred plants to a controlled environment with 20 degree celcius temperature and 100 % RH
...
```{r}
f1 <- plot(ggpredict(mod4, "total_rain"))
f2 <- plot(ggpredict(mod4 , "rain_duration"))
f3 <- plot(ggpredict(mod4, "wind_speed"))
