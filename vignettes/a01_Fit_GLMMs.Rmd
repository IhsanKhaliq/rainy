 ...
 * set the wind speed to `NA` when there is no rainfall,
...
dat <- read_csv(system.file(
  ...
  mutate(
    `AVERAGE Wind Speed km/h` = if_else(`TOTAL Rain Gauge mm` > 0,
  ...
  summarise(
    ...
    wind_speed = round(
      mean(`AVERAGE Wind Speed km/h`, na.rm = TRUE) * 0.277778,
                       1) # convert to m/s
  ) %>%
